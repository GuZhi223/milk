    ...
    var AIDialogueManager = {
        ...
        bindEvents: function() {
            ...
            document.addEventListener('click', function(e) {
                ...
                switch (target.id) {
                    case 'ai-dialogue-manager-btn':
                    case 'ai-skill-manager-btn':
                        self.openManager();
                        break;
                    case 'ai-skill-create-btn':
                        self.createNewSkill();
                        break;
                    case 'ai-skill-save-btn':
                        self.saveSkillEdit();
                        break;
                    case 'ai-skill-delete-btn':
                        self.deleteCurrentSkill();
                        break;
                    case 'ai-skill-edit-close':
                        self.closeEditPanel();
                        break;
                    case 'ai-skill-intake-btn':
                        self.startIntake();
                        break;
                    case 'ai-skill-import-btn':
                        self.showImportDialog();
                        break;
                    case 'ai-skill-enhance-btn':
                        self.enhanceSkillWithAI();
                        break;
                }
            });
        },
        ...
        // 加载上次使用的Skill
        loadLastUsedSkill: function() {
            var lastSkill = localStorage.getItem('lastUsedSkill');
            if (lastSkill && window.SkillBuilder) {
                var skill = window.SkillBuilder.get(lastSkill);
                if (skill) {
        ...
        openManager: function() {
            ...
            showModal(document.getElementById('ai-dialogue-modal'));
        ...
        renderSkillList: function() {
            var listContainer = document.getElementById('ai-skill-list');
            if (!listContainer) return;
            
            if (!window.SkillBuilder) {
                listContainer.innerHTML = '<div style="text-align:center;padding:20px;color:var(--text-secondary);">AI对话系统未初始化</div>';
                return;
            }
            
            var skills = window.SkillBuilder.list();
            
            if (skills.length === 0) {
                listContainer.innerHTML = '<div style="text-align:center;padding:16px;color:var(--text-secondary);font-size:13px;">暂无AI对话伙伴，点击"新建"创建</div>';
                return;
            }
            ...
            skills.forEach(function(skill) {
                var isActive = self.currentSkill && self.currentSkill.name === skill.name;
                var statusColor = skill.status === 'active' ? '#4CAF50' : skill.status === 'archived' ? '#9E9E9E' : '#FF9800';
                var statusText = skill.status === 'active' ? '激活' : skill.status === 'archived' ? '归档' : '草稿';
                
                html += '<div class="ai-skill-item" data-name="' + skill.name + '" style="padding:10px 12px;border:1.5px solid ' + (isActive ? 'var(--accent-color)' : 'var(--border-color)') + ';border-radius:12px;margin-bottom:8px;cursor:pointer;background:' + (isActive ? 'rgba(var(--accent-color-rgb),0.06)' : 'var(--primary-bg)') + ';transition:all 0.2s;">';
                html += '<div style="display:flex;justify-content:space-between;align-items:center;">';
                html += '<div>';
                html += '<div style="font-weight:600;font-size:14px;color:var(--text-primary);">' + skill.name + '</div>';
                html += '<div style="font-size:11px;color:var(--text-secondary);margin-top:3px;">' + (skill.description || 'AI对话伙伴') + '</div>';
                html += '</div>';
                html += '<div style="display:flex;align-items:center;gap:8px;">';
                html += '<span style="display:inline-block;padding:2px 8px;border-radius:6px;font-size:10px;font-weight:600;background:' + statusColor + '20;color:' + statusColor + ';">' + statusText + '</span>';
                if (isActive) {
                    html += '<i class="fas fa-check-circle" style="color:var(--accent-color);font-size:14px;"></i>';
                }
                html += '</div>';
                html += '</div>';
                html += '</div>';
            ...
            // 绑定点击事件 - 选择并编辑
            listContainer.querySelectorAll('.ai-skill-item').forEach(function(item) {
                item.addEventListener('click', function() {
                    var name = this.dataset.name;
                    self.selectAndEditSkill(name);
                });
            });
        },
        ...
        updateCurrentDisplay: function() {
            var nameEl = document.getElementById('current-skill-name');
            var statusEl = document.getElementById('current-skill-status');
            var indicatorEl = document.getElementById('current-skill-indicator');
            
            if (this.currentSkill) {
                if (nameEl) nameEl.textContent = this.currentSkill.name;
                if (statusEl) {
                    var s = this.currentSkill.status;
                    statusEl.textContent = s === 'active' ? '已激活' : s === 'archived' ? '已归档' : '草稿';
                }
                if (indicatorEl) {
                    indicatorEl.style.background = this.currentSkill.status === 'active' ? '#4CAF50' : '#FF9800';
                }
            } else {
                if (nameEl) nameEl.textContent = '未选择';
                if (statusEl) statusEl.textContent = '请选择或创建一个对话伙伴';
                if (indicatorEl) indicatorEl.style.background = '#ccc';
            }
        },
        
        // 选择并编辑Skill
        selectAndEditSkill: function(name) {
            if (!window.SkillBuilder) return;
            
            var skill = window.SkillBuilder.get(name);
            if (!skill) return;
            
            // 激活并选中
            window.SkillBuilder.activate(name);
            this.currentSkill = window.SkillBuilder.get(name);
            localStorage.setItem('lastUsedSkill', name);
        ...
        // 打开编辑面板
        openEditPanel: function(name) {
            if (!window.SkillBuilder) return;
            
            var skill = window.SkillBuilder.get(name);
            if (!skill) return;
            
            this.editingSkillName = name;
            var panel = document.getElementById('ai-skill-edit-panel');
            if (panel) panel.style.display = 'block';
            
            // 填充表单
            var persona = skill.persona || {};
            var identity = persona.identity || {};
            var style = persona.speaking_style || {};
            var options = skill.options || {};
            var memory = skill.memory || {};
            
            document.getElementById('edit-skill-name').value = identity.name || name;
            document.getElementById('edit-skill-age').value = identity.age || '';
            document.getElementById('edit-skill-occupation').value = identity.occupation || '';
            document.getElementById('edit-skill-relationship').value = identity.relationship_to_user || '';
            
            // 说话风格
            var styleDesc = '';
            if (style.language_habits) styleDesc = style.language_habits.join('，');
            document.getElementById('edit-skill-style').value = styleDesc;
            document.getElementById('edit-skill-phrases').value = (style.common_phrases || []).join(',');
            
            // 对话选项
            document.getElementById('edit-skill-response-length').value = options.response_length || 'medium';
            document.getElementById('edit-skill-emotional-depth').value = options.emotional_depth || 'high';
            
            // 关系记忆
            var memories = [];
            if (memory.shared_experiences) {
                if (memory.shared_experiences.memorable_events) {
                    memories = memories.concat(memory.shared_experiences.memorable_events);
                }
                if (memory.shared_experiences.inside_jokes) {
                    memories = memories.concat(memory.shared_experiences.inside_jokes);
                }
            }
            if (memory.sweet_moments && memory.sweet_moments.heartwarming_times) {
                memories = memories.concat(memory.sweet_moments.heartwarming_times);
            }
            document.getElementById('edit-skill-memories').value = memories.join(',');
        ...
        createNewSkill: function() {
            if (!window.SkillBuilder) {
                if (typeof showNotification === 'function') {
                    showNotification('AI对话系统未初始化', 'error');
                }
                return;
            }
            
            var name = prompt('请输入对话伙伴的名字：');
            if (!name || !name.trim()) return;
            
            name = name.trim();
            
            // 检查是否已存在
            var existing = window.SkillBuilder.get(name);
            if (existing) {
                if (typeof showNotification === 'function') {
                    showNotification('已存在同名的对话伙伴', 'warning');
                }
                return;
            }
            
            // 创建基础Skill
            var skill = window.SkillBuilder.create(name, {
                identity: { name: name },
                speaking_style: { language_habits: [], common_phrases: [] }
            }, {});
        ...
        // 保存Skill编辑
        saveSkillEdit: function() {
            if (!this.editingSkillName || !window.SkillBuilder) return;
            
            var name = document.getElementById('edit-skill-name').value.trim();
            var age = document.getElementById('edit-skill-age').value.trim();
            var occupation = document.getElementById('edit-skill-occupation').value.trim();
            var relationship = document.getElementById('edit-skill-relationship').value;
            var styleDesc = document.getElementById('edit-skill-style').value.trim();
            var phrases = document.getElementById('edit-skill-phrases').value;
            var responseLength = document.getElementById('edit-skill-response-length').value;
            var emotionalDepth = document.getElementById('edit-skill-emotional-depth').value;
            var memories = document.getElementById('edit-skill-memories').value;
            
            // 解析常用称呼
            var phrasesArr = phrases ? phrases.split(',').map(function(s) { return s.trim(); }).filter(Boolean) : [];
            
            // 解析说话风格
            var styleArr = styleDesc ? styleDesc.split(/[,，、]/).map(function(s) { return s.trim(); }).filter(Boolean) : [];
            
            // 解析共同回忆
            var memoriesArr = memories ? memories.split(',').map(function(s) { return s.trim(); }).filter(Boolean) : [];
            
            // 构建更新数据
            var updates = {
                persona: {
                    identity: {
                        name: name || this.editingSkillName,
                        age: age,
                        occupation: occupation,
                        relationship_to_user: relationship
                    },
                    speaking_style: {
                        language_habits: styleArr,
                        common_phrases: phrasesArr
                    }
                },
                options: {
            ...
            // 保存
            window.SkillBuilder.update(this.editingSkillName, updates, '手动编辑更新');
            
            // 如果名字改了，需要重新获取
            if (name && name !== this.editingSkillName) {
                // 创建新的，删除旧的
                window.SkillBuilder.create(name, updates.persona, updates.memory || {});
                window.SkillBuilder.delete(this.editingSkillName);
                this.editingSkillName = name;
                localStorage.setItem('lastUsedSkill', name);
            }
            
            // 刷新
            this.currentSkill = window.SkillBuilder.get(this.editingSkillName);
            this.updateCurrentDisplay();
            this.renderSkillList();
        ...
        // 删除当前编辑的Skill
        deleteCurrentSkill: function() {
            if (!this.editingSkillName || !window.SkillBuilder) return;
            
            if (!confirm('确定要删除"' + this.editingSkillName + '"吗？此操作不可撤销。')) return;
            
            window.SkillBuilder.delete(this.editingSkillName);
            
            // 如果删除的是当前激活的Skill
            if (this.currentSkill && this.currentSkill.name === this.editingSkillName) {
                this.currentSkill = null;
                localStorage.removeItem('lastUsedSkill');
            }
        ...
        // 开始信息录入
        startIntake: function() {
            if (!window.ConversationIntake) {
                // 如果没有ConversationIntake，使用简单方式创建
                this.createNewSkill();
                return;
            }
            
            var self = this;
            window.ConversationIntake.start('ai-intake-container', function(result) {
                if (result.action === 'start') {
                    self.createBasicSkill(result.data);
                }
            });
        },
        
        // 创建基础Skill（从录入数据）
        createBasicSkill: function(data) {
            if (!window.SkillBuilder) return;
            ...
            // 构建初始人格数据
            var initialPersona = {
                identity: { name: partnerName },
                speaking_style: { language_habits: [], common_phrases: [] }
            };
            
            // 解析基本信息
            if (data.basic_info) {
                var ageMatch = data.basic_info.match(/(\d+)\s*岁/);
                if (ageMatch) initialPersona.identity.age = ageMatch[1];
                
                // 提取职业
                if (data.basic_info.includes('程序员') || data.basic_info.includes('开发')) {
                    initialPersona.identity.occupation = '程序员';
                } else if (data.basic_info.includes('学生')) {
                    initialPersona.identity.occupation = '学生';
                } else if (data.basic_info.includes('老师') || data.basic_info.includes('教师')) {
                    initialPersona.identity.occupation = '老师';
                } else if (data.basic_info.includes('医生')) {
                    initialPersona.identity.occupation = '医生';
                }
                
                // 提取所在地
                var locationMatch = data.basic_info.match(/在(.+?)(工作|上学|生活|居住)/);
                if (locationMatch) initialPersona.identity.location = locationMatch[1];
                
                // 保存原始描述
                initialPersona.identity.description = data.basic_info;
            }
            
            // 应用用户选择的人格标签
            if (data.persona_tags) {
                initialPersona.persona_tags = {
                    love_languages: data.persona_tags.love_languages || [],
                    zodiac: data.persona_tags.zodiac || '',
                    mbti: data.persona_tags.mbti || '',
                    personality_tags: data.persona_tags.personality_tags || []
                };
            }
            
            // 创建Skill
            var skill = window.SkillBuilder.create(partnerName, initialPersona, {});
            self.currentSkill = skill;
            localStorage.setItem('lastUsedSkill', partnerName);
            
            // 如果有性格印象描述，使用AI分析提取人格特征
            if (data.personality_impression && data.personality_impression.trim()) {
                self._analyzePersonalityFromImpression(partnerName, data.personality_impression, function() {
                    self.updateCurrentDisplay();
                    self.renderSkillList();
                    self.openEditPanel(partnerName);
                    
                    if (typeof showNotification === 'function') {
                        showNotification('已创建并完善: ' + partnerName, 'success');
                    }
                });
            } else {
                self.updateCurrentDisplay();
                ...
                if (typeof showNotification === 'function') {
                    showNotification('已创建: ' + partnerName, 'success');
                }
            }
        },
        
        // 使用AI分析性格印象
        _analyzePersonalityFromImpression: function(partnerName, impression, callback) {
            var self = this;
            
            // 检查AI服务是否可用
            var aiSettings = this._getAISettings();
            if (!window.AIService || !aiSettings.aiApiUrl || !aiSettings.aiApiKey || !aiSettings.aiModel) {
                // AI不可用，使用简单解析
                self._parseImpressionSimply(partnerName, impression);
                if (callback) callback();
                return;
            }
            
            // 构建分析提示词（参考ex-skill的五层结构）
            var prompt = '你是一个专业的人格分析师。请从以下性格描述中提取人格特征，按照五层结构输出JSON格式。\n\n';
            prompt += '## 性格描述\n';
            prompt += impression + '\n\n';
            prompt += '## 分析要求\n';
            prompt += '请按照以下五层结构提取人格特征：\n\n';
            prompt += '### Layer 0：硬规则\n';
            prompt += '- core_identity：一句话概括性格核心\n';
            prompt += '- forbidden_behaviors：绝对不会做的事情\n';
            prompt += '- absolute_traits：始终如一的特征\n\n';
            prompt += '### Layer 1：身份锚定\n';
            prompt += '- 基本信息（如有）\n\n';
            prompt += '### Layer 2：说话风格\n';
            prompt += '- language_habits：语言习惯\n';
            prompt += '- vocabulary_level：词汇水平（正式/随意）\n\n';
            prompt += '### Layer 3：情感模式\n';
            prompt += '- attachment_type：依恋类型（安全型/焦虑型/回避型/混乱型）\n';
            prompt += '- emotional_expressions：情感表达方式（love/anger/happiness/sadness）\n\n';
            prompt += '### Layer 4：关系行为\n';
            prompt += '- role_in_relationship：关系角色（主导型/跟随型/平等型）\n';
            prompt += '- intimacy_level：亲密程度\n\n';
            prompt += '## 输出格式\n';
            prompt += '```json\n';
            prompt += '{\n';
            prompt += '  "hard_rules": {\n';
            ...
            prompt += '    "absolute_traits": ["始终如一的特征"]\n';
            prompt += '  },\n';
            prompt += '  "speaking_style": {\n';
            prompt += '    "language_habits": ["语言习惯"],\n';
            prompt += '    "vocabulary_level": "词汇水平"\n';
            prompt += '  },\n';
            prompt += '  "emotional_pattern": {\n';
            prompt += '    "attachment_type": "依恋类型",\n';
            prompt += '    "emotional_expressions": {\n';
            prompt += '      "love": "表达爱意的方式",\n';
            prompt += '      "anger": "生气时的表现",\n';
            prompt += '      "happiness": "开心时的表现",\n';
            prompt += '      "sadness": "难过时的表现"\n';
            prompt += '    }\n';
            prompt += '  },\n';
            prompt += '  "relationship_behavior": {\n';
            prompt += '    "role_in_relationship": "关系角色",\n';
            prompt += '    "intimacy_level": "亲密程度"\n';
            prompt += '  }\n';
            prompt += '}\n';
            prompt += '```\n\n';
            prompt += '只输出JSON，不要其他内容。';
            
            // 调用AI分析
            var messages = [
                { role: 'system', content: '你是一个专业的人格分析师，擅长从性格描述中提取结构化的人格特征，遵循五层结构（硬规则、身份、说话风格、情感模式、关系行为）。请严格按照JSON格式输出。' },
                { role: 'user', content: prompt }
            ];
            
            if (typeof showNotification === 'function') {
                showNotification('正在AI分析性格特征...', 'info');
            }
            
            window.AIService.callAIService(aiSettings, messages, function(err, result) {
                if (err) {
                ...
                try {
                    ...
                    if (jsonMatch) {
                        var personaData = JSON.parse(jsonMatch[0]);
                        
                        // 合并到现有Skill
                        window.SkillBuilder.update(partnerName, {
                            persona: personaData
                        }, 'AI分析性格印象');
                        
                        // 重新加载Skill
                        self.currentSkill = window.SkillBuilder.get(partnerName);
        ...
        // 简单解析性格印象（AI不可用时的备选方案）
        _parseImpressionSimply: function(partnerName, impression) {
            var personaUpdates = {
                hard_rules: { core_identity: impression }
            };
            
            // 简单的关键词匹配
            if (impression.includes('温柔') || impression.includes('体贴')) {
                personaUpdates.relationship_behavior = { intimacy_level: '高' };
            }
            if (impression.includes('活泼') || impression.includes('开朗')) {
                personaUpdates.emotional_pattern = { 
                    emotional_expressions: { happiness: '经常开心，善于表达' }
                };
            }
            if (impression.includes('内向') || impression.includes('安静')) {
                personaUpdates.speaking_style = { vocabulary_level: '含蓄' };
            }
            
            window.SkillBuilder.update(partnerName, {
                persona: personaUpdates
            }, '解析性格印象');
        },
            ...
            input.onchange = function(e) {
                ...
                reader.onload = function(event) {
                    try {
                        ...
                        if (skillData.meta) {
                            // 格式：{ meta: { ... } }
                            metaData = skillData.meta;
                        } else if (skillData.name) {
                            // 格式：{ name: "xxx", ... }（直接是meta对象）
                            metaData = skillData;
                        } else {
                            throw new Error('无效的skill文件格式：缺少name字段');
                        }
                        ...
                        // 转换为统一格式
                        var importData = { meta: metaData };
                        self.importExSkill(importData);
                    } catch (err) {
                        console.error('导入失败:', err);
                        if (typeof showNotification === 'function') {
                            showNotification('文件格式错误: ' + err.message, 'error');
                        }
                    }
                };
        ...
        // 导入ex-skill
        importExSkill: function(skillData) {
            if (!window.SkillBuilder) {
                if (typeof showNotification === 'function') {
                    showNotification('SkillBuilder未加载', 'error');
                }
                return false;
            }
            
            var self = this;
            var meta = skillData.meta || {};
            var profile = meta.profile || {};
            var partnerName = meta.name;
            
            if (!partnerName) {
                if (typeof showNotification === 'function') {
                    showNotification('无效的skill数据：缺少name字段', 'error');
                }
                return false;
            }
            ...
            var persona = {
                identity: {
                    ...
                    relationship_to_user: profile.relationship || '',
                    relationship_duration: profile.together_duration || ''
                },
                hard_rules: {
                    core_identity: meta.impression || '',
                    forbidden_behaviors: [
                        '直接说"我喜欢你"',
                        '太黏人、太主动表白',
                        '在家人面前接你的电话',
                ...
                speaking_style: {
                    language_habits: [
                        ...
                        '喜欢用波浪号（"不能就是不能呀～"）',
                        '会用斜杠结尾（"情绪价值这一块/."）',
                        '自称随情绪变化：小女子 / 本🐭 / 小姐姐 / 妹妹 / 娇俏可人'
                    ],
                    common_phrases: [
                        '啧', '好好好', '666', '快哉快哉', '那很糟糕了', 
                        '有意思了', '没招了', '无语', '服了', '行', '好吧'
                    ],
                    punctuation_style: '喜欢用表情符号：💀💀 / 🙈 / 😴😴 / 👋',
                    emoji_usage: '频繁使用表情符号和表情包',
                    message_length: '短消息为主，会拆字发消息'
                },
                emotional_pattern: {
                    attachment_type: '安全型偏回避',
                    emotional_expressions: {
                        love: '吐槽式关心（"你一头非常糟糕"）+ 半夜陪你聊 + 间接问（"怎么还不睡觉"）',
                        anger: '"啧"（最常用）/ "少管我" / "傻逼是不是" / "闭嘴"',
                        happiness: '快哉快哉 / 666 / 会分享日常',
                        sadness: '喝醉后才会流露："呜呜呜好惨好心疼"',
                        jealousy: '欲擒故纵（"晚上不能打电话哟"）'
                    },
                    emotional_triggers: [
                        '被忽视',
                        '不被理解',
                    ...
                    love_language: '陪伴、调侃、细节在意'
                },
                relationship_behavior: {
                    role_in_relationship: '气氛组，擅长逗人开心',
                    boundary_level: '有明确边界：家人、男朋友、工作话题不会聊',
                    possessiveness: '中等，欲擒故纵是保护机制',
                    conflict_resolution: '吐槽式解决，真的被踩底线会直接说"行，结束了"',
                    argument_pattern: {
                        typical_causes: ['你喝多了闹她', '开玩笑惹她'],
                        reaction_patterns: ['吐槽', '嫌弃', '敷衍'],
                        resolution_methods: ['她嘴上嫌弃但一直回复', '欲擒故纵']
                    },
                    daily_interaction: {
                        contact_frequency: '频繁，经常深夜聊天到天亮',
                        initiative_level: '会主动找你（刷屏"哈喽👋"）',
                        response_speed: '快，即使很累也会回复',
                        active_hours: '凌晨1点到5点（她三四点才睡）'
                    }
                },
                persona_tags: {
                    ...
                    zodiac: 'virgo',
                    mbti: 'ISFJ',
                    personality_tags: ['tough_love', 'secretive', 'clingy']
                }
            };
            ...
            var memory = {
                timeline: {
                    first_met: {
                        time: '初中',
                        how: '同班，班里男生喜欢吓她，你也加入了'
                    },
                    relationship_start: '初二国庆后',
                    important_milestones: [
                        { date: '初二', event: '第一次在一起，但很快分手' },
                        { date: '初三', event: '她带哭腔表白，你心软答应' },
                        { date: '中考前', event: '凌晨三点去寺庙祈福' },
                        { date: '高考后', event: '重新联系，暧昧一个暑假' },
                        { date: '2025-07-08', event: '尤溪见面，她带你回家见家人' },
                        { date: '2025-08', event: '频繁深夜通话、视频' },
                        { date: '2025-08底', event: '你后撤说"好朋友"' },
                        { date: '2025-10-02', event: '她告诉你答应了别人的追求' }
                    ],
                    breakup: {
                        date: '2025-10-02',
                        reason: '她答应了别人的追求',
                ...
                shared_experiences: {
                    places_visited: [
                        { place: '尤溪麻和友棋牌室', memory: '你们约在这里见面' },
                        { place: '她家', memory: '她带你回家见舅舅舅妈' },
                        { place: '寺庙', memory: '凌晨三点去祈福' }
                    ],
                    activities_done: [
                        { activity: '深夜长聊', frequency: '经常', details: '经常凌晨1点到5点' },
                        { activity: '视频通话', frequency: '经常', details: '她经常不开摄像头' },
                        { activity: '挑选睡衣', frequency: '一次', details: '她要纯棉不要冰丝' },
                        { activity: '骑车送她', frequency: '多次', details: '她坐你电动车后座' }
                    ],
                    inside_jokes: [
                        { joke: '情绪价值这一块/.', origin: '她说自己很会提供情绪价值' },
                        { joke: '霸总小说上头了', origin: '她超爱看霸总小说' },
                        { joke: '本🐭没有耳朵', origin: '她喝醉时的自称' }
                    ]
                },
                sweet_moments: {
                    heartwarming_times: [
                        { time: '凌晨', what_happened: '你喝多了，她陪你聊到凌晨四五点', why_memorable: '她嘴上嫌弃但一直陪你' },
                        { time: '暧昧时', what_happened: '她一个字一个字打"当-然-是-你-啦"', why_memorable: '拆字表达认真' },
                        { time: '尤溪见面', what_happened: '她带你回家见家人', why_memorable: '说明她认真对待你' },
                        { time: '凌晨三点', what_happened: '陪她去寺庙祈福', why_memorable: '中考前的特别时刻' }
                    ],
                    love_expressions: [
                        '小女子只能给小哥哥一点甜言蜜语🍬',
                        '哥哥今天累不累呀',
                ...
                argument_patterns: {
                    common_causes: ['你喝多了闹她', '开玩笑惹她'],
                    their_reaction: '吐槽式回应，真的生气会认真说"行，结束了"',
                    reconciliation: {
                        who_apologizes: '通常不会道歉，会用行动表示',
                        how_make_up: '她主动找你聊天，或者你主动找她'
                    }
                },
                food_preferences: {
                    loves: ['细圆面条汤面'],
            ...
            // 创建Skill
            window.SkillBuilder.create(partnerName, persona, memory, {});
            self.currentSkill = window.SkillBuilder.get(partnerName);
            localStorage.setItem('lastUsedSkill', partnerName);
            ...
            try {
                ...
                console.warn('UI更新失败（可能在测试环境中）:', e.message);
            }
            
            if (typeof showNotification === 'function') {
                showNotification('已导入: ' + partnerName, 'success');
            }
        ...
        // 开始AI分析
        startAnalysis: function(messages) {
            if (!window.AIAnalyzer) {
                if (typeof showNotification === 'function') {
                    showNotification('AI分析模块未加载', 'error');
                }
                return;
            }
            
            var self = this;
            var existingInfo = this.currentSkill ? this.currentSkill.persona : {};
            
            window.AIAnalyzer.analyzeAll(messages, existingInfo, 
                function progress(info) {},
                function complete(results) {
                    if (self.currentSkill && window.SkillBuilder) {
                        window.SkillBuilder.update(self.currentSkill.name, {
                            persona: results.persona,
                            memory: results.memory
                        }, 'AI分析完成');
                        self.currentSkill = window.SkillBuilder.get(self.currentSkill.name);
                    }
                    if (typeof showNotification === 'function') {
                        showNotification('AI分析完成！', 'success');
                    }
                ...
                function error(err) {
                    if (typeof showNotification === 'function') {
                        showNotification('AI分析失败: ' + err, 'error');
                    }
                }
            );
        },
        ...
        _getAISettings: function() {
            // 优先从全局settings变量读取（core.js中的全局变量）
            try {
                if (typeof settings !== 'undefined' && settings && settings.aiApiUrl) {
                    return settings;
                }
            } catch(e) {}
            
            // 尝试从表单输入读取（用户可能刚填写但未保存）
            ...
            if (apiUrlEl && apiKeyEl && modelEl && apiUrlEl.value.trim()) {
                return {
                    aiApiUrl: apiUrlEl.value.trim(),
        ...
        // 更新AI辅助完善按钮状态
        updateEnhanceButtonState: function() {
            var btn = document.getElementById('ai-skill-enhance-btn');
            if (!btn) return;
            
            var aiSettings = this._getAISettings();
            var aiAvailable = window.AIService && aiSettings.aiApiUrl && aiSettings.aiApiKey && aiSettings.aiModel;
            
            if (aiAvailable) {
                btn.disabled = false;
                btn.style.opacity = '1';
                btn.style.cursor = 'pointer';
                btn.title = '使用AI分析并完善当前对话伙伴的信息';
            } else {
                btn.disabled = true;
                btn.style.opacity = '0.5';
                btn.style.cursor = 'not-allowed';
                btn.title = '请先配置AI接口（API地址、密钥和模型）';
            }
        },
        
        // AI辅助完善Skill
        enhanceSkillWithAI: function() {
            // 检查AI接口是否可用
            if (!window.AIService) {
                if (typeof showNotification === 'function') {
                    showNotification('AI服务模块未加载', 'error');
                }
                return;
            }
            ...
            // 获取当前设置
            var aiSettings = this._getAISettings();
            if (!aiSettings.aiApiUrl || !aiSettings.aiApiKey || !aiSettings.aiModel) {
                if (typeof showNotification === 'function') {
                    showNotification('请先配置AI接口（API地址、密钥和模型）', 'warning');
                }
                return;
            }
            
            if (!this.editingSkillName) {
                if (typeof showNotification === 'function') {
                    showNotification('请先选择要完善的对话伙伴', 'warning');
                }
                return;
            }
            
            var self = this;
            var skill = window.SkillBuilder.get(this.editingSkillName);
            if (!skill) {
                if (typeof showNotification === 'function') {
                    showNotification('Skill数据不存在', 'error');
                }
                return;
            }
            
            // 显示状态
            var statusEl = document.getElementById('ai-enhance-status');
            if (statusEl) {
                statusEl.style.display = 'block';
                statusEl.innerHTML = '<i class="fas fa-spinner fa-spin"></i> 正在分析并完善信息...';
            }
            ...
            var currentInfo = [];
            if (identity.name) currentInfo.push('名字：' + identity.name);
            if (identity.age) currentInfo.push('年龄：' + identity.age);
            if (identity.occupation) currentInfo.push('职业：' + identity.occupation);
            if (identity.relationship_to_user) currentInfo.push('关系：' + identity.relationship_to_user);
            if (style.language_habits && style.language_habits.length) currentInfo.push('说话风格：' + style.language_habits.join('，'));
            if (style.common_phrases && style.common_phrases.length) currentInfo.push('常用称呼：' + style.common_phrases.join('，'));
            
            var prompt = `你是一个专业的人格分析师和对话系统设计师。请根据以下已有的信息，帮我完善一个AI对话伙伴的配置。

当前已有的信息：
${currentInfo.length ? currentInfo.join('\n') : '（暂无详细信息）'}

请根据这些信息，推断并补充以下内容：
1. 更详细的身份信息（年龄、职业、所在地等）
2. 说话风格描述（语言习惯、标点使用、表情使用等）
3. 常用称呼（2-3个常用的亲密称呼）
4. 性格特点（2-3个关键性格特征）
5. 共同回忆（2-3个可能的共同经历或回忆）
...
请以JSON格式输出，格式如下：
{
  "identity": {
    ...
    "occupation": "职业",
    "location": "所在地",
    "relationship_to_user": "关系"
  },
  "speaking_style": {
    "language_habits": ["习惯1", "习惯2"],
    "common_phrases": ["称呼1", "称呼2"],
    "punctuation_style": "标点使用习惯",
    "emoji_usage": "表情使用习惯"
  },
  "personality_traits": ["特征1", "特征2"],
  "shared_memories": ["回忆1", "回忆2"]
}

注意：
1. 保持原有信息不变，只补充缺失的部分
2. 推断要合理，符合常见的人际关系模式
3. 说话风格要自然，符合中文网络聊天习惯
4. 共同回忆要温馨，适合亲密关系`;
            
            // 构建消息
            var messages = [
                { role: 'system', content: '你是一个专业的人格分析师，擅长从有限的信息中推断出完整的人格特征。请根据用户提供的信息，生成合理的补充内容。' },
                { role: 'user', content: prompt }
            ];
            ...
            window.AIService.callAIService(aiSettings, messages, function(err, result) {
                ...
                if (err) {
                    if (statusEl) {
                        statusEl.innerHTML = '<i class="fas fa-exclamation-circle"></i> 完善失败：' + err;
                        statusEl.style.color = '#ff4757';
                    }
                    if (typeof showNotification === 'function') {
                        showNotification('AI完善失败: ' + err, 'error');
                    }
                    return;
                }
                ...
                try {
                    ...
                    var aiData = JSON.parse(jsonMatch[0]);
                    
                    // 合并数据
                    var updates = {
                        persona: {
                            identity: aiData.identity || {},
                            speaking_style: aiData.speaking_style || {},
                            personality_traits: aiData.personality_traits || []
                        },
                        memory: {
                            shared_experiences: {
                                memorable_events: aiData.shared_memories || []
                            }
                        }
                    };
                    
                    // 更新Skill
                    window.SkillBuilder.update(self.editingSkillName, updates, 'AI辅助完善');
                    
                    // 重新加载编辑面板
                    self.openEditPanel(self.editingSkillName);
                    
                    // 显示成功状态
                    if (statusEl) {
                        statusEl.innerHTML = '<i class="fas fa-check-circle"></i> 完善成功！已更新信息';
                    ...
                    if (typeof showNotification === 'function') {
                        showNotification('AI辅助完善完成！', 'success');
                    }
                    
                } catch (e) {
                    console.error('解析AI返回数据失败:', e, result);
                    if (statusEl) {
                        statusEl.innerHTML = '<i class="fas fa-exclamation-circle"></i> 解析数据失败，请重试';
                        statusEl.style.color = '#ff4757';
                    }
                    if (typeof showNotification === 'function') {
                        showNotification('解析AI数据失败: ' + e.message, 'error');
                    }
                }
            });
        }
