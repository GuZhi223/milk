...
function _tryRecoverFromBackup() {
    try {
        ...
        // 检查备份是否属于当前会话，防止跨会话数据污染
        if (backup && backup.sessionId && backup.sessionId !== SESSION_ID) {
            console.warn(`[backup] 备份属于会话 ${backup.sessionId}，当前会话是 ${SESSION_ID}，跳过恢复`);
            return null;
        }
        return backup;
    } catch (e) {
...
const addMessage = (message) => {
    ...
    // 返回实际添加到 DOM 的消息 wrapper 元素（用于流式更新等场景）
    addedElement = container.lastElementChild;
    // 如果最后是日期分隔符，取其前一个兄弟节点
    if (addedElement && addedElement.classList.contains('date-divider')) {
        addedElement = addedElement.previousElementSibling;
    }
...
if (!isBatchMode && type === 'normal') {
    ...
    window._pendingReplyTimer = null;
        ...
        window.simulateReply = function() {
            ...
            // ── AI模式分支 ──
            if (settings.aiEnabled && window.AIService && settings.aiApiKey && settings.aiApiUrl && settings.aiModel) {
                // 先处理拍一拍和persona切换（3%概率拍一拍仍保留）
                if (Math.random() < 0.03) {
                    if (typeof window._triggerPartnerPoke === 'function') window._triggerPartnerPoke();
                    return;
                }
                ...
                if (partnerPersonas && partnerPersonas.length > 0 && Math.random() < 0.3) {
                    var currentPool = [...partnerPersonas];
                    if (currentPool.length > 0) {
                        var nextPersona = currentPool[Math.floor(Math.random() * currentPool.length)];
                ...
                // 获取当前激活的Skill名称，使用带Skill版本构建上下文
                var currentSkillName = (window.AIDialogueManager && window.AIDialogueManager.currentSkill) 
                    ? window.AIDialogueManager.currentSkill.name : null;
                var contextMsgs = currentSkillName 
                    ? AIService.buildContextMessagesWithSkill(settings, messages, currentSkillName)
                    : AIService.buildContextMessages(settings, messages);

                if (settings.aiStreamEnabled) {
                    // 流式输出：直接操作 DOM，不走 addMessage 避免渲染冲突
                    ...
                    function createStreamBubble() {
                        ...
                        wrapper.dataset.id = replyId;
                        wrapper.dataset.msgId = replyId;
                        ...
                        try {
                            ...
                            avatarDiv.innerHTML = partnerImg ? '<img src="' + partnerImg.src + '">' : '<i class="fas fa-user"></i>';
                        } catch(e) { avatarDiv.innerHTML = '<i class="fas fa-user"></i>'; }
                        ...
                        metaEl.textContent = new Date().toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' });
                        ...
                        // 插入到 spacer 前或末尾
                        var spacer = container.querySelector('div[style*="flex: 1"]');
                        if (spacer && spacer === container.lastElementChild) {
                        ...
                        // 记录到 messages 数组（用于 saveData 持久化）
                        messages.push({
                            id: replyId,
                    ...
                    AIService.callAIServiceStream(settings, contextMsgs,
                        function onChunk(chunk) {
                            replyText += chunk;
                            if (!streamBubbleEl) {
                                streamBubbleEl = createStreamBubble();
                            }
                            var textEl = streamBubbleEl.querySelector('.message-text');
                            if (textEl) textEl.textContent = replyText;
                            // 同步内存数据
                            for (var mi = 0; mi < messages.length; mi++) {
                                if (String(messages[mi].id) === String(replyId)) {
                                    messages[mi].text = replyText;
                                    break;
                                }
                            }
                            requestAnimationFrame(function() {
                        ...
                        function onDone() {
                            ...
                            if (replyText) {
                                ...
                                if (typeof window._sendPartnerNotification === 'function') {
                                    window._sendPartnerNotification(settings.partnerName || '对方', replyText);
                                }
                                throttledSaveData();
                            }
                        },
                        function onError(errMsg) {
                            console.warn('[AI Stream Error]', errMsg);
                            hideTypingIndicator();
                            if (streamBubbleEl) streamBubbleEl.remove();
                            messages = messages.filter(function(m) { return String(m.id) !== String(replyId); });
                            throttledSaveData();
                            if (typeof showNotification === 'function') {
                                showNotification('AI回复失败，已切换到字卡模式: ' + errMsg, 'warning', 3000);
                            }
                            simulateReply_Fallback();
                        }
                    );
                } else {
                    // 非流式输出
                    AIService.callAIService(settings, contextMsgs, function(err, result) {
                        hideTypingIndicator();
                        if (err) {
                            console.warn('[AI Error]', err);
                            if (typeof showNotification === 'function') {
                                showNotification('AI回复失败，已切换到字卡模式: ' + err, 'warning', 3000);
                            }
                            simulateReply_Fallback();
                            return;
                        }
                        if (result) {
                            ...
                            playSound('message');
                            if (typeof window._sendPartnerNotification === 'function') {
                                window._sendPartnerNotification(settings.partnerName || '对方', result);
                            }
                            throttledSaveData();
                        }
                    });
            ...
            // ── 以下为原有字卡模式逻辑 ──
            // 包装为fallback函数供AI模式降级使用
            function simulateReply_Fallback() {
            // 原有逻辑继续...
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // ── AI设置面板事件绑定 ──
    var aiSettingsCard = document.getElementById('ai-settings');
    if (aiSettingsCard) {
        aiSettingsCard.addEventListener('click', function() {
            hideModal(document.getElementById('settings-modal'));
            showModal(document.getElementById('ai-settings-modal'));
            loadAISettingsUI();
        });
    }

    // ── AI对话伙伴管理按钮事件绑定 ──
    var aiSkillManagerBtn = document.getElementById('ai-skill-manager-btn');
    if (aiSkillManagerBtn) {
        aiSkillManagerBtn.addEventListener('click', function() {
            if (window.AIDialogueManager) {
                window.AIDialogueManager.openManager();
            }
        });
    }
    ...
    function loadAISettingsUI() {
        ...
        var apiUrl = document.getElementById('ai-api-url');
        var apiKey = document.getElementById('ai-api-key');
        var model = document.getElementById('ai-model');
        var sysPrompt = document.getElementById('ai-system-prompt');
        var maxCtx = document.getElementById('ai-max-context');
        var maxTokens = document.getElementById('ai-max-tokens');
        var temp = document.getElementById('ai-temperature');
        var tempVal = document.getElementById('ai-temp-val');
        var streamToggle = document.getElementById('ai-stream-toggle');
        var exSkillToggle = document.getElementById('use-ex-skill-prompt');
        var exSkillNotice = document.getElementById('ex-skill-prompt-notice');

        if (toggle) toggle.checked = !!settings.aiEnabled;
        if (apiUrl) apiUrl.value = settings.aiApiUrl || '';
        if (apiKey) apiKey.value = settings.aiApiKey || '';
        if (model) model.value = settings.aiModel || '';
        if (sysPrompt) sysPrompt.value = settings.aiSystemPrompt || '';
        if (maxCtx) maxCtx.value = settings.aiMaxContextMessages || 20;
        if (maxTokens) maxTokens.value = settings.aiMaxTokens || 512;
        if (temp) temp.value = settings.aiTemperature !== undefined ? settings.aiTemperature : 0.8;
        if (tempVal) tempVal.textContent = temp ? temp.value : '0.8';
        if (streamToggle) streamToggle.checked = settings.aiStreamEnabled !== false;
        
        // ex-skill提示词开关
        if (exSkillToggle) {
            exSkillToggle.checked = !!settings.useExSkillPrompt;
            // 更新UI状态
            _updateExSkillPromptUI(exSkillToggle.checked);
            // 添加事件监听器
            exSkillToggle.addEventListener('change', function() {
                _updateExSkillPromptUI(this.checked);
            });
        }
    ...
    // 更新ex-skill提示词UI状态
    function _updateExSkillPromptUI(enabled) {
        var sysPrompt = document.getElementById('ai-system-prompt');
        var exSkillNotice = document.getElementById('ex-skill-prompt-notice');
        
        if (sysPrompt) {
            sysPrompt.disabled = enabled;
            sysPrompt.style.opacity = enabled ? '0.5' : '1';
            sysPrompt.style.cursor = enabled ? 'not-allowed' : 'text';
        }
        
        if (exSkillNotice) {
            exSkillNotice.style.display = enabled ? 'block' : 'none';
        }
    }

    function highlightActivePreset() {
        var cards = document.querySelectorAll('.ai-preset-card');
        cards.forEach(function(card) {
            var presetKey = card.getAttribute('data-preset');
            var preset = window.AI_PRESETS ? window.AI_PRESETS[presetKey] : null;
            if (preset && settings.aiApiUrl === preset.apiUrl && settings.aiModel === preset.model) {
                card.style.borderColor = 'var(--accent-color)';
                card.style.background = 'linear-gradient(135deg, rgba(var(--accent-color-rgb),0.12), rgba(var(--accent-color-rgb),0.04))';
            } else {
    ...
    // 预设卡片点击
    var presetCards = document.querySelectorAll('.ai-preset-card');
    presetCards.forEach(function(card) {
        card.addEventListener('click', function() {
            ...
            var apiUrl = document.getElementById('ai-api-url');
            var model = document.getElementById('ai-model');
            if (apiUrl) apiUrl.value = preset.apiUrl;
    ...
    if (tempSlider) {
        tempSlider.addEventListener('input', function() {
            var tempVal = document.getElementById('ai-temp-val');
            if (tempVal) tempVal.textContent = this.value;
        });
    }
    ...
    if (testBtn) {
        testBtn.addEventListener('click', function() {
            ...
            if (!apiKey.value.trim() || !apiUrl.value.trim() || !model.value.trim()) {
                if (resultDiv) {
                    resultDiv.style.display = 'block';
                    resultDiv.style.background = 'rgba(255,59,48,0.1)';
                    resultDiv.style.color = '#FF3B30';
                    resultDiv.textContent = '请先填写完整的API配置';
                }
                return;
            }
            ...
            if (resultDiv) {
                resultDiv.style.display = 'block';
                resultDiv.style.background = 'rgba(var(--accent-color-rgb),0.1)';
            ...
            if (window.AIService) {
                AIService.testAIConnection(testSettings, function(success, msg) {
                    testBtn.disabled = false;
                    testBtn.innerHTML = '<i class="fas fa-plug"></i> 测试连接';
                    if (resultDiv) {
                        if (success) {
                            resultDiv.style.display = 'block';
                            resultDiv.style.background = 'rgba(52,199,89,0.1)';
                            resultDiv.style.color = '#34C759';
                            resultDiv.textContent = '✓ 连接成功！AI回复: ' + (msg.length > 80 ? msg.substring(0, 80) + '...' : msg);
                        } else {
                            resultDiv.style.display = 'block';
                            resultDiv.style.background = 'rgba(255,59,48,0.1)';
                            resultDiv.style.color = '#FF3B30';
                            resultDiv.textContent = '✗ 连接失败: ' + msg;
                        }
                    }
                });
            }
    ...
    // 保存AI设置
    var saveAiBtn = document.getElementById('save-ai-settings');
    if (saveAiBtn) {
        saveAiBtn.addEventListener('click', function() {
            ...
            settings.aiEnabled = toggle ? toggle.checked : false;
            settings.aiApiUrl = apiUrl ? apiUrl.value.trim() : '';
            settings.aiApiKey = apiKey ? apiKey.value.trim() : '';
            settings.aiModel = model ? model.value.trim() : '';
            settings.aiSystemPrompt = sysPrompt ? sysPrompt.value.trim() : '';
            settings.aiMaxContextMessages = maxCtx ? parseInt(maxCtx.value) || 20 : 20;
            settings.aiMaxTokens = maxTokens ? parseInt(maxTokens.value) || 512 : 512;
            settings.aiTemperature = temp ? parseFloat(temp.value) || 0.8 : 0.8;
            settings.aiStreamEnabled = streamToggle ? streamToggle.checked : true;
            settings.useExSkillPrompt = exSkillToggle ? exSkillToggle.checked : false;

            throttledSaveData();
            showNotification('AI设置已保存 ✓', 'success', 2000);
        });
    }
});
