    ...
    if (annToggleBtn && annFormWrapper) {
        ...
        async function deleteSession(sessionId) {
            ...
            if (confirm('确定要删除此会话及其所有聊天记录吗？此操作不可恢复')) {
                ...
                // Bug 4: await 保存会话列表，避免加载错位
                sessionList = sessionList.filter(s => s.id !== sessionId);
                try {
                    await localforage.setItem(`${APP_PREFIX}sessionList`, sessionList);
                } catch (err) {
                    showNotification('保存会话列表失败', 'error');
                    console.error('保存会话列表失败:', err);
                    return;
                }

                // 同步清除 localStorage 中该会话的所有键
                Object.keys(localStorage).forEach(key => {
                    if (key.startsWith(`${APP_PREFIX}${sessionId}_`)) safeRemoveItem(key);
                });

                // Bug 2: await 清理 localforage 中该会话的键
                try {
                    const keys = await localforage.keys();
                    await Promise.all(keys
                        .filter(key => key.startsWith(`${APP_PREFIX}${sessionId}_`))
                        .map(key => localforage.removeItem(key).catch(() => {}))
                    );
                } catch (err) {
                    console.error('清理 localforage 键失败:', err);
                }

                if (sessionId === currentSessionId) {
                    const newCurrentId = sessionList[0].id;
                    // Bug 1: 移除冗余的 customThemes 写入
        ...
        DOMElements.sessionModal.list.addEventListener('click', (e) => {
            ...
            } else if (e.target.closest('.session-info')) {
                // 点击会话名称区域
